...
class myVisio(BertEmbeddings):
    def __init__(self, config, *args, **kwargs):
        ...
        self.w1 = torch.nn.Parameter(torch.zeros(config.obj_cls,config.hidden_size))
...
#         visual_embeddings = self.projection(visual_embeddings)
        ...
        #print(self.w1.size())
#         print(config)
        result = torch.mm(visual_embeddings.view(-1, self.config.obj_cls), self.w1)
        #hardcode 100
        visual_embeddings = result.view(-1, 100, self.config.hidden_size)
#         visual_embeddings = visual_embeddings*self.w1
